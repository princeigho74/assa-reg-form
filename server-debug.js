...
// Register new member with enhanced error handling
app.post('/api/register', registrationValidation, async (req, res) => {
  ...
  try {
    ...
    console.log('Processing registration for:', { firstName, surname, email, graduationYear, termsAccepted });

    // Additional server-side validation for 2006 graduation year
    const year = parseInt(graduationYear);
    if (year !== 2006) {
      console.log('Invalid graduation year:', year);
      return res.status(400).json({
        success: false,
...
app.listen(PORT, () => {
  console.log('=================================');
  console.log(`🚀 ASSA 2006 Set Registration Server`);
